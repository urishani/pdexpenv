 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class WizardControl4MultAreaMod extends AbstractWizardControl {
	 ...
	 * @param pTable
	 */
	public WizardControl4MultAreaMod(MyProperties pProperties, View pView, Table pTable) {
	...
	int tableRows;
	int tableCols;
	int rowStart, rowEnd, colStart, colEnd, multiplyBy;
	...
	protected String createCode(AbstractHtmlPanel pPanel) 
	{
		String code = 
			...
			"         table[ row, column] := table[ row, column] * " + multiplyBy + ";\n" +
			"      next column;\n" +
			"   next row;\n" +
			"\n";
		return code;
	...
	protected boolean verify(AbstractHtmlPanel pPanel) 
	{
		...
		try 
		{
			...
			multiplyBy = getIntParam(pPanel, "multiplyBy", new Integer(0), null);
		} catch (Exception e) 	
		{
